...
int cdrom_exec_cmd(int cmd, void *param) {
    ...
    switch (n) {
        case COMPLETED: return ERR_OK;
        case ABORTED: return ERR_ABORTED;
        case NO_ACTIVE: return ERR_NO_ACTIVE;
        default: {
            switch(status[0]) {
                case 2: return ERR_NO_DISC;
                case 6: return ERR_DISC_CHG;
                default: return ERR_SYS;
            }
        }
    }
}
...
int pread(void *buf, unsigned int nbyte, unsigned int offset) {
    ...
    if (offset>=fh.len) return 0;
    
    /* If the full read would span beyond the EOF, shorten the read */
    if (offset+nbyte > fh.len) nbyte = fh.len - offset;
